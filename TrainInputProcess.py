...
class TrainInputProcess:
    def __init__(self,
        ...
        elif self.text_model_type == 'roberta' or self.text_model_type == 'roberta-large':
            self.tokenizer = AutoTokenizer.from_pretrained(self.text_model, add_prefix_space=True)
            # self.tokenizer.add_special_tokens({'pad_token': '[PAD]'})
        elif self.text_model_type == 'neobert':
            self.tokenizer = AutoTokenizer.from_pretrained(self.text_model, trust_remote_code=True)
            # self.tokenizer.add_special_tokens({'pad_token': '[PAD]'})

        self.image_process = ViTImageProcessor.from_pretrained(self.image_model, use_fast=True)

    def get_text_dataset_from_tsv(self, process_label=False):
        """从 TSV 读取数据（与 image2text_conversion / TwitterDataMABSA-main 格式一致）。
        data_text_dir 下需有 train.tsv、dev.tsv、test.tsv。train/dev 列: #1 Label, #2 ImageID, #3 String, #3 String；
        test 列: #1 ImageID 或 #1 Label, #2 String, #2 String。"""
        for dataset_type in self.dataset_types:
            tsv_path = os.path.join(self.data_text_dir, f"{dataset_type}.tsv")
            if not os.path.exists(tsv_path):
                return False
            df = pd.read_csv(tsv_path, sep="\t")
            # 统一列名：train/dev 用 #1 Label, #2 ImageID, #3 String；test 用 #1 ImageID, #2 String
            if dataset_type == "test":
                # test.tsv 表头多为 index, #1 ImageID, #2 String, #2 String；数据列顺序常为：index, sentiment, image_id(.jpg), tweet_content, target
                col_list = list(df.columns)
                for i in range(min(len(col_list), 5)):
                    sample = str(df.iloc[0, i])
                    if ".jpg" in sample or ".png" in sample:
                        df["image_id"] = df.iloc[:, i].astype(str)
                        break
                if "image_id" not in df.columns and len(col_list) >= 3:
                    df["image_id"] = df.iloc[:, 2].astype(str)
                if "#2 String" in df.columns:
                    df["tweet_content"] = df["#2 String"]
                if "#2 String.1" in df.columns:
                    df["target"] = df["#2 String.1"]
                if len(col_list) >= 5:
                    if "tweet_content" not in df.columns:
                        df["tweet_content"] = df.iloc[:, 3].astype(str)
                    if "target" not in df.columns:
                        df["target"] = df.iloc[:, 4].astype(str)
                if "sentiment" not in df.columns:
                    if "#1 ImageID" in df.columns:
                        df["sentiment"] = pd.to_numeric(df["#1 ImageID"], errors="coerce").fillna(1).astype(int)
                    elif len(col_list) >= 2:
                        df["sentiment"] = pd.to_numeric(df.iloc[:, 1], errors="coerce").fillna(1).astype(int)
                    else:
                        df["sentiment"] = 1
                if "target" not in df.columns:
                ...
                if "#3 String.1" in df.columns:
                    df = df.rename(columns={"#3 String.1": "target"})
                elif "target" not in df.columns and len(df.columns) >= 5:
                    df["target"] = df.iloc[:, -1]
            sentence_l = []
            image_l = []
            label_l = []
            ...
            for _, row in df.iterrows():
                tweet = str(row.get("tweet_content", ""))
                target = str(row.get("target", ""))
                image_id = str(row.get("image_id", ""))
                sentiment = int(row.get("sentiment", 1))
                if sentiment not in (0, 1, 2):
                    sentiment = 1
                tokens = tweet.split()
                if "$T$" not in tokens:
                    tokens = tweet.replace("$T$", " $T$ ").split()
                if "$T$" not in tokens:
                    continue
                start_pos = tokens.index("$T$")
                aspect_tokens = target.split() if target else ["$T$"]
                end_pos = start_pos + len(aspect_tokens) - 1
                if end_pos >= len(tokens):
                    end_pos = len(tokens) - 1
                n_key = len(tokens)
                s_label = [0] * n_key
                s_label[start_pos] = sentiment + 2
                for i in range(start_pos + 1, min(end_pos + 1, n_key)):
                    s_label[i] = 1
                v_sentiment = sentiment
                pair_l.append([(f"{start_pos}-{end_pos}", v_sentiment)])
                sentence_l.append(tokens)
                image_l.append(image_id)
    ...
    def generate_input(self):
        if self.train_type == 0:
            # 优先从 TSV 读取（Tweets15_corenlp 下 train/dev/test.tsv），否则从 .txt 读取
            if not self.get_text_dataset_from_tsv():
    ...
    def generate_im2t_input(self):
        ...
        for dataset_type in self.dataset_types:
            ...
            for image_path in image_l:
                ...
                # 检查图片文件是否存在，如果不存在则使用占位符
                if os.path.exists(image_file_path):
                    image = Image.open(image_file_path)
                    image = image.convert('RGB')
                else:
                    # 创建占位符图片：RGB 模式，224x224 白色图片
                    print(f"⚠️  警告: 图片文件不存在，使用占位符: {image_file_path}")
    ...
    def generate_dualc_input(self):
        for dataset_type in self.dataset_types:
            ...
            with open("datasets/finetune/dualc/{}/input_{}.txt".format(self.nianfen, dataset_type), "w",
...
def main():
    ...
    # 仅使用服务器本地已下载的两个模型：
    # 文本模型：/root/data/weights/roberta-large
    # 图像模型：/root/data/weights/vit-large-patch16-224
    if text_model_type in ('roberta', 'roberta-large'):
        text_model = "/root/data/weights/roberta-large"
    elif text_model_type == 'bert':
        text_model = "/root/data/weights/bert-large"
    else:
        raise ValueError(f"不支持的 text_model_type: {text_model_type}，请使用 roberta 或 roberta-large")

    if image_model_type in ('vit', 'vit-large'):
        image_model = "/root/data/weights/vit-large-patch16-224"
    elif image_model_type == 'swin':
        image_model = "/root/data/weights/swin"
    else:
        raise ValueError(f"不支持的 image_model_type: {image_model_type}，请使用 vit 或 vit-large")
    ...
    if finetune_task == 'im2t':
        ...
        if image_gen_model_type == 'ved':
            ...
            image_gen_text_model = "/root/data/weights/blip-large"
    elif finetune_task == 'clipc':
        image_model = "/root/data/weights/clipc-large"

    data_text_dir = None
    data_image_dir = None
    # 在服务器 ~/data/ICT-main 下，文本和图片真实目录为：
    # dataset/Tweets15_corenlp, dataset/Tweets17_corenlp, dataset/twitter2015_images, dataset/twitter2017_images
    # 这里使用相对路径，确保在 ~/data/ICT-main 目录下运行脚本时可以正确找到数据。
    if dataset_type == '2015':
        data_text_dir = 'dataset/Tweets15_corenlp'
        data_image_dir = 'dataset/twitter2015_images'
    elif dataset_type == '2017':
        data_text_dir = 'dataset/Tweets17_corenlp'
        data_image_dir = 'dataset/twitter2017_images'
